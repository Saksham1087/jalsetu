...
- [x] 9.1 Fix map height — conditionally remove `pb-24` from main on map tab (`src/App.jsx`)
- [x] 9.2 Add `ResizeObserver` + multi-delay `invalidateSize()` for robust map sizing on mobile (`src/components/PublicMap.jsx`)
- [x] 9.3 Remove conflicting `.leaflet-control-zoom a` size overrides (`src/styles/map.css`)
- [x] 9.4 Redesign filter panel as bottom sheet drawer with drag handle, swipe-to-dismiss, tap-outside overlay, and slide animation (`src/components/PublicMap.jsx`)
- [ ] 9.5 Verify map renders full height with no gray areas on mobile viewport
- [ ] 9.6 Verify filter bottom sheet opens/closes with smooth animation on mobile
